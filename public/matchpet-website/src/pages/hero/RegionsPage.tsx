      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 mt-24 sm:mt-32 md:mt-56">
        <div className="mx-auto max-w-2xl lg:max-w-none">
          ...
          <div className="overflow-x-auto no-scrollbar h-12 gap-3 flex flex-row">
            <button onClick={() => {changeLocation({id: "3107", nome: "Metropolitana de Belo Horizonte", UF: {id: "31", sigla: "MG", nome: "Minas Gerais", regiao: {id: "3", sigla: "SE", nome: "Sudeste"}}}); navigate(searchParams.get('to') || '/animals')
}} className='px-2 flex-shrink-0 overflow-hidden min-w-42 min-h-full h-full bg-neutral border-border rounded-xl border-2 flex justify-center items-center'>
              Região Metropolitana BH 
            </button>
            <button onClick={() => {changeLocation({id: "3515", nome: "Metropolitana de S\u00e3o Paulo", UF: {id: "35", sigla: "SP", nome: "S\u00e3o Paulo", regiao: {id: "3", sigla: "SE", nome: "Sudeste"}}}); navigate(searchParams.get('to') || '/animals')
}} className='px-2 flex-shrink-0 overflow-hidden min-w-42 min-h-full h-full bg-neutral border-border rounded-xl border-2 flex justify-center items-center'>
              Região Metropolitana SP
            </button>
            <button onClick={() => {changeLocation({id: "3306", nome: "Metropolitana do Rio de Janeiro", UF: {id: "33", sigla: "RJ", nome: "Rio de Janeiro", regiao: {id: "3", sigla: "SE", nome: "Sudeste"}}}); navigate(searchParams.get('to') || '/animals')
}} className='px-2 flex-shrink-0 overflow-hidden min-w-42 min-h-full h-full bg-neutral border-border rounded-xl border-2 flex justify-center items-center'>
              Região Metropolitana do RJ
            </button>
            <button onClick={() => {changeLocation({id: "2905", nome: "Metropolitana de Salvador", UF: {id: "29", sigla: "BA", nome: "Bahia", regiao: {id: "2", sigla: "NE", nome: "Nordeste"}}}); navigate(searchParams.get('to') || '/animals')
}} className='px-2 flex-shrink-0 overflow-hidden min-w-42 min-h-full h-full bg-neutral border-border rounded-xl border-2 flex justify-center items-center'>
          ...
          <button onClick={chosenMesoRegion? () => {changeLocation(chosenMesoRegion); navigate(searchParams.get('to') || '/animals')
}: () => {}}  className="primary-button mt-8 h-12 max-w-xl w-full">Confirmar</button>
        </div>
      </div>
    </div>
  )
